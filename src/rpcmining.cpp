...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    nExpectedTime = staking ? (GetTargetSpacing(nNetworkWeight, true) * nNetworkWeight / nWeight) : 0;
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Force is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Force is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Force is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Force is downloading blocks...");
