...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeForceAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeForceAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeForceAmount = strAnonymizeForceAmount.remove(strAnonymizeForceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeForceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeForceAmount*COIN) nMaxToAnonymize = nAnonymizeForceAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeForceAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeForceAmount));
        strAnonymizeForceAmount = strAnonymizeForceAmount.remove(strAnonymizeForceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeForceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
