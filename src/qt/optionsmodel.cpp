...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeForceAmount"))
        settings.setValue("nAnonymizeForceAmount", 1000);
    nAnonymizeForceAmount = settings.value("nAnonymizeForceAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeForceAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeForceAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeForceAmount:
            return QVariant(nAnonymizeForceAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeForceAmount:
            nAnonymizeForceAmount = value.toInt();
            settings.setValue("nAnonymizeForceAmount", nAnonymizeForceAmount);
            emit AnonymizeForceAmountChanged(nAnonymizeForceAmount);
            break;
        default:
            break;
        }
