...
LIBS += $$PWD/src/secp256k1/src/libsecp256k1_la-secp256k1.o
    # we use QMAKE_CXXFLAGS_RELEASE even without RELEASE=1 because we use RELEASE to indicate linking preferences not -O preferences
    gensecp256k1.commands = cd $$PWD/src/secp256k1 && ./autogen.sh && ./configure --enable-module-recovery --with-bignum=no && CC=$$QMAKE_CC CXX=$$QMAKE_CXX $(MAKE) OPT=\"$$QMAKE_CXXFLAGS $$QMAKE_CXXFLAGS_RELEASE\"
...
isEmpty(GMP_LIB_PATH) {
    macx:GMP_LIB_PATH = /usr/local/Cellar/6.1.2_2/lib
}
...
isEmpty(OPENSSL_INCLUDE_PATH) {
    macx:OPENSSL_INCLUDE_PATH = /usr/local/openssl/1.0.2s/include
...
macx:ICON = src/qt/res/icons/Force.icns
macx:TARGET = "Force-Qt"
